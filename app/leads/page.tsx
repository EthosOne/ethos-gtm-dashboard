...
export default function LeadsPage() {
  ...
  const [linkedinDayFilter, setLinkedinDayFilter] = useState<string | null>(null);
  ...
  const [linkedinDmOnly, setLinkedinDmOnly] = useState(false);
  ...
  const loadContacts = useCallback(async () => {
    ...
    if (smeOnly) q = q.eq("icp_tier", "SME");
    if (unsubOnly) q = q.not("twlr_unsubscribed_at", "is", null);
    ...
    if (linkedinDmOnly) q = q.ilike("notes", `%linkedin_connect_sent:${linkedinDayFilter ?? ""}%`);
    ...
    setLoading(false);
  }, [stage, page, pageSize, search, twlrOnly, engagedOnly, gdprOnly, smeOnly, unsubOnly, linkedinOnly, joOnly, episodeBuilderOnly, affiliateOnly, referredOnly, linkedinDmOnly, linkedinDayFilter, listFilter, sortField, sortDir]);
  ...
  const loadCounts = useCallback(() => {
    ...
    supabase.from("contacts").select("*", { count: "exact", head: true }).eq("icp_tier", "SME")
      .then(({ count }) => setSmeCount(count ?? 0));
    supabase.from("contacts").select("*", { count: "exact", head: true }).not("twlr_unsubscribed_at", "is", null)
      .then(({ count }) => setUnsubCount(count ?? 0));
    supabase.from("contacts").select("*", { count: "exact", head: true }).not("linkedin_url", "is", null)
      .then(({ count }) => setLinkedinCount(count ?? 0));
    supabase.from("contacts").select("*", { count: "exact", head: true }).eq("list_name", "LinkedIn Prospects - JO")
    ...
    supabase.from("contacts").select("notes,linkedin_url").ilike("notes", "%linkedin_connect_sent%").limit(2000)
      .then(({ data }) => {
        // Dedupe by linkedin_url — some contacts are loaded 2-3x from different
        // enrichment batches (same person, same URL, different row id).
        const byDay: Record<string, Set<string>> = {};
        const seenTotal = new Set<string>();
        for (const row of data ?? []) {
          const url = row.linkedin_url ?? row.notes; // fallback key if no url
          seenTotal.add(url);
        ...
        setLinkedinDmCount(seenTotal.size);
        setLinkedinByDay(Object.entries(byDay).map(([d, s]) => [d, s.size] as [string, number]).sort((a, b) => b[0].localeCompare(a[0])));
      });
  }, []);
    ...
    <main style={{ background: t.bg, minHeight: "100vh", padding: 0, transition: "background 0.3s", overflowX: "hidden" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: "1.25rem" }}>
          ...
          <button onClick={() => { setTwlrOnly(v => !v); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setEngagedOnly(v => !v); setTwlrOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setLinkedinOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setGdprOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setLinkedinOnly(false); setSmeOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setSmeOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setUnsubOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setJoOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setEpisodeBuilderOnly(v => !v); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setAffiliateOnly(v => !v); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setReferredOnly(v => !v); setAffiliateOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setListFilter(""); setStage("All"); setPage(0); }} style={{
          ...
          <button onClick={() => { setLinkedinDmOnly(v => !v); setLinkedinDayFilter(null); setReferredOnly(false); setAffiliateOnly(false); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setJoOnly(false); setUnsubOnly(false); setEpisodeBuilderOnly(false); setListFilter(""); setStage("All"); setPage(0); }} style={{
            background: linkedinDmOnly ? "#3A5F8A22" : t.surface,
            border: `1px solid ${linkedinDmOnly ? "#2A4A6B66" : t.border}`,
            color: linkedinDmOnly ? "#2A4A6B" : t.textMuted,
            borderRadius: 999, padding: "5px 13px", cursor: "pointer",
            fontSize: "0.78rem", fontWeight: 700, fontFamily: "inherit",
            letterSpacing: "0.03em", transition: "all 0.15s",
          }}>
            LinkedIn Outreach{linkedinDmCount > 0 ? <span style={{ marginLeft: 5, opacity: 0.65 }}>({linkedinDmCount.toLocaleString()})</span> : null}{linkedinDmOnly && " ✓"}
          </button>
          {listOptions.length > 0 && (
            <select
              value={listFilter}
              onChange={e => { setListFilter(e.target.value); setTwlrOnly(false); setEngagedOnly(false); setGdprOnly(false); setSmeOnly(false); setLinkedinOnly(false); setUnsubOnly(false); setJoOnly(false); setEpisodeBuilderOnly(false); setAffiliateOnly(false); setReferredOnly(false); setLinkedinDmOnly(false); setLinkedinDayFilter(null); setStage("All"); setPage(0); }}
              style={{
        ...
        {linkedinDmOnly && linkedinByDay.length > 0 && (
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14, fontSize: "0.75rem" }}>
            {linkedinByDay.map(([day, n]) => {
              const active = linkedinDayFilter === day;
              return (
                <button key={day}
                  onClick={() => { setLinkedinDayFilter(active ? null : day); setPage(0); }}
                  style={{
                    background: active ? "#3A5F8A22" : t.surfaceAlt,
                    border: `1px solid ${active ? "#2A4A6B66" : t.border}`,
                    color: active ? "#2A4A6B" : t.textMuted,
                    borderRadius: 6, padding: "3px 9px", cursor: "pointer", fontFamily: "inherit", fontSize: "0.75rem",
                  }}>
                  {new Date(day + "T00:00:00").toLocaleDateString("es-BO", { day: "2-digit", month: "short" })}: <b style={{ color: active ? "#2A4A6B" : t.text }}>{n}</b>{active && " ✓"}
                </button>
              );
            })}
          </div>
